  ...
  <form @submit.prevent="onSubmit" class="table-container">
    <table :tableHeader="tableHeader">
      ...
      <tr v-for="(tData,index) in groupedTableData" :key="tData.ev">
        ...
        <td>
          <input
            class="w-20" :disabled="tempo !== 'volume'"
            type="number"
            v-model="tData[0].litres">
        </td>
      </tr>
    </table>
...
import { toInteger } from 'lodash';
    ...
    for (let iFor = 0; iFor < dataStore.evConfigLength / 5; iFor++) {
      ...
      if (dataStore.evConfig.hasOwnProperty('S' + evIndex)) {
        ...
        switch (tempo.value) {
          ...
          case 'volume':
            console.log('volume')
            let tmpLitres = dataStore.evStation === undefined ? undefined : dataStore.evStation['S' + (((optionValue.value - 1) * 1000) + (evStationIndex1+(dataStore.evConfig['S' + (evIndex+2)]-1)))]
            litres.value = tmpLitres === undefined ? 0 : tmpLitres
            break;
        }
        ...
        postEvStationData.value.payload['S' + (((optionValue.value - 1) * 1000) + (evStationIndex1+(dataStore.evConfig['S' + (evIndex+2)]-1)))] = String(evData.value[i].litres)
      i++  
      }
      } else {
        //i--
  ...
  function updatePostData() {
    ...
    for (let iFor = 0; iFor < dataStore.evConfigLength / 5; iFor++) {
      ...
      if (dataStore.evConfig.hasOwnProperty('S' + evIndex)) {
        ...
        postEvStationData.value.payload['S' + (((optionValue.value - 1) * 1000) + (evStationIndex1+(dataStore.evConfig['S' + (evIndex+2)]-1)))] = String(groupedTableData.value[(dataStore.evConfig['S' + (evIndex+2)])][0].litres)
        }
      } else {
        //i--
      }
  ...
  const tableHeader = ['N Stazione', 'Serial code ev', 'Ordine stazione', 'Identificativo', 'litres']
